...
export const createTutorial =
  tutorialData => async (firebase, firestore, dispatch, history) => {
    try {
      ...
      const { title, summary, owner, created_by, is_org, featured_video } =
        tutorialData;
        ...
        if (featured_video) {
          try {
            ...
            const videoRef = storageRef.child(
              `tutorial_videos/${documentID}_${featured_video.name}`
            );
            await videoRef.put(featured_video);
            videoUrl = await videoRef.getDownloadURL();
          } catch (error) {
            console.log("Error uploading video to Firebae Storage:", error);
          }
        }
