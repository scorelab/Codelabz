...
export const fetchTutorialTags =
  tutorialId => async (firebase, firestore, dispatch) => {
    try {
      const tutorialDoc = await firestore
        ...
        .doc(tutorialId)
        .get();
      const tags = tutorialDoc.data().tags || [];
      return tags;
    } catch (error) {
      console.log(error);
      return [];
...
export const updateTutorialTags =
  (tutorialId, newTags) => async (firebase, firestore, dispatch) => {
    try {
      // dispatch({ type: actions.UPDATE_TUTORIAL_TAGS_START });
      await firestore.collection("tutorials").doc(tutorialId).update({
        tags: newTags,
        updatedAt: firestore.FieldValue.serverTimestamp()
      });

      // dispatch({
      //   type: actions.UPDATE_TUTORIAL_TAGS_SUCCESS,
      //   payload: { tutorialId, newTags }
      // });
    } catch (error) {
      // dispatch({
      //   type: actions.UPDATE_TUTORIAL_TAGS_FAIL,
